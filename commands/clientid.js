...
module.exports.run = async (bot, message, args, prefixes) => {
  ...
  var serverlist =  bot.guilds.cache.map(r => r.name + r.id + ` | ${r.memberCount} Membres`)
        let min = 0;
        let max = 10;
        let guilds = await message.channel.send(`${serverlist}`)
        await guilds.react("◀");
        await guilds.react("⏹");
        await guilds.react("▶");
        ...
        collector.on("collect", async(reaction) => {
            ...
            await reaction.remove(message.author.id).catch((err) => {
                if(err) {
                    return message.author.send(`Une erreur est survenue :\n\`\`\`${err}\`\`\``);
                }
            });
        });
    };
